...
function boot(router) {
  ...
  const fmtDateTime = (d) => {
    // loggedAt comes back from Postgres as a full ISO timestamp already (not a
    // plain date), so parse it directly — this also converts UTC to whatever
    // timezone the viewer's browser is in, same as any other JS Date display.
    if (!d) return "—";
    try {
      const dt = new Date(d);
      return dt.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) +
  ...
  function renderJobsList() {
    ...
    wrap.innerHTML = sorted.map((j) => {
      ...
      return (
        '<div class="job-card">' +
          ...
          '<button class="btn small ghost" data-edit-job="' + j.id + '">Edit</button>' +
    ...
    wrap.querySelectorAll("[data-edit-job]").forEach((btn) => {
  ...
  function renderDashboard() {
    ...
    wrap.innerHTML = summary.map((c) => {
      ...
      return (
        '<div class="cat-card"><div class="cat-card-head">' +
        '<div class="cat-name" data-filter-category="' + c.category + '" title="View PO lines in this category" role="button" tabindex="0">' + c.category + '</div>' +
  ...
  function renderInvoices() {
    const job = activeJob();
    if (!job) {
      document.getElementById("inv-tbody").innerHTML = '<tr><td colspan="7"><div class="empty-state"><div class="big">No jobs yet</div>Head to the All Jobs tab and click "+ New Job".</div></td></tr>';
      return;
    }
    const search = document.getElementById("inv-search").value.trim().toLowerCase();
    let rows = [...job.invoices];
    if (search) rows = rows.filter((r) => r.po.toLowerCase().includes(search) || (r.vendor || "").toLowerCase().includes(search) || (r.invoiceNum || "").toLowerCase().includes(search));
    rows.sort((a, b) => {
      const k = invSort.key;
      let av = a[k], bv = b[k];
    ...
    if (rows.length === 0) {
      tbody.innerHTML = '<tr><td colspan="7"><div class="empty-state"><div class="big">No invoices logged yet</div>Click "Log Invoice" to add the first one.</div></td></tr>';
    ...
    tbody.innerHTML = pageRows.map((r) => (
      '<tr><td class="mono">' + fmtDate(r.date) + '</td>' +
      '<td class="mono" style="color:var(--ink-soft);font-size:11.5px;">' + fmtDateTime(r.loggedAt) + '</td>' +
      '<td class="mono">' + r.po + '</td>' +
  ...
  document.getElementById("btn-new-job-fromtab").addEventListener("click", () => openJobModal(null));
  ...
  document.getElementById("job-save").addEventListener("click", async () => {
    ...
    try {
      if (editId) {
        await api("/api/jobs", { method: "PATCH", body: JSON.stringify({ id: editId, ...payload }) });
        jobModal.classList.remove("open");
        await refresh();
        toast(name + " updated");
      } else {
        const { id } = await api("/api/jobs", { method: "POST", body: JSON.stringify(payload) });
        state.activeJobId = id;
        jobModal.classList.remove("open");
        await refresh();
        switchTab("dashboard");
  ...
  function exportToExcel() {
    const job = activeJob();
    if (!job) { toast("Create a job first"); return; }
    // job.pos comes straight from the database with no guaranteed row order —
    // sort it the same way Cost Detail and the PO search box already do, so the
    // export always lands in PO # order instead of whatever order Postgres felt
    // like returning that day.
    const sortedPos = [...job.pos].sort((a, b) => a.po.localeCompare(b.po, undefined, { numeric: true }));
    const poRows = sortedPos.map((p) => {
      const cost = costForPO(job, p.po);
      return { "PO #": p.po, "Type": p.type, "Category": p.category, "Budget": p.budget, "Cost to Date": cost, "Balance": (Number(p.budget) || 0) - cost };
    });
    const sortedInvoices = [...job.invoices].sort((a, b) => (a.date || "").localeCompare(b.date || ""));
    const invRows = sortedInvoices.map((i) => ({
      "Date": i.date || "", "PO Code": i.po, "Vendor": i.vendor || "", "Invoice #": i.invoiceNum || "",
      "Amount": i.amount, "Source": i.source || "", "Logged At": i.loggedAt ? new Date(i.loggedAt) : ""
    }));
    ...
    ws2["!cols"] = [{ wch: 13 }, { wch: 12 }, { wch: 24 }, { wch: 14 }, { wch: 14 }, { wch: 12 }, { wch: 19 }];
    setColFormat(ws2, "A", 2, invRows.length + 1, XLSX_DATE_FMT, true);
    setColFormat(ws2, "E", 2, invRows.length + 1, XLSX_MONEY_FMT, false);
    setColFormat(ws2, "G", 2, invRows.length + 1, "yyyy-mm-dd hh:mm AM/PM", false);
  ...
  (async function init() {
    try {
      ...
      populateCatFilter();
      renderAll();
      switchTab("jobs"); // always land on All Jobs after a fresh load — never assume which job the user wants
