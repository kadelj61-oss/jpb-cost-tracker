...
export async function ensureSchema() {
  ...
  schemaReady = (async () => {
    ...
    `;
    // For databases created before this column existed — ADD COLUMN IF NOT EXISTS
    // backfills existing rows with now() at migration time, and new inserts get
    // the real logged time automatically via the column default above.
    await sql`ALTER TABLE invoices ADD COLUMN IF NOT EXISTS logged_at TIMESTAMPTZ DEFAULT now();`;
    await sql`
